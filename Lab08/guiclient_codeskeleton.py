...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.ipPort.insert(tk.END, DEFAULT_HOST)
...
# set the state of the program to 'disconnected'
def disconnect():
...
def tryToConnect():
    ...
    try:
        ...
        tkmsgbox.showerror("Connect ERROR", str(error))
...
def sendMessage(master):
    ...
    if g_bConnected:
        try:
            g_sock.sendall(bytearray(master.textIn.get(), "ASCII"))
            master.textIn.delete(0, tk.END)
        except socket.error as error:
            disconnect()
            tkmsgbox.showerror("Message ERROR", str(error))
            # if a socket.error occurrs, you may want to disconnect, in order
            # to put the program into a defined state
    else:
        tkmsgbox.showwarning("Server connection missing", "Please, establish connection with the server before sending messages...")
...
def pollMessages():
    ...
    if g_bConnected:
        try:
            ...
            if not data:
                printToMessages("You have been kicked from the chat")
                disconnect()
                return
