...
episodes = re.findall(r"<td class=\"svtTablaTime\">\s+"
                      ...
                      r"<p class=\"svtXMargin-Bottom-10px\">\s+(.*?)\s+"
                      r"</p>", txt)
for record in episodes:
    tid = record[0]
    description = record[1]
    descSplit = re.findall(r"Amerikansk animerad komediserie från [\d-]+\. Säsong (\d+)\. Del (\d+) av (\d+)\.(.+)", description)[0]
    season = descSplit[0]
    episode = descSplit[1]+"\\"+descSplit[2]
    description = descSplit[3]
    print("-"*50)
    print("""
        Tid: {}
        Säsong: {}
